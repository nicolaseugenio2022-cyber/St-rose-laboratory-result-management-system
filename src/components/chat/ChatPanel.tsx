...
/**
 * The expanded body of the support chat: the conversation, the composer, the streaming
 * transport and the assistant-content renderer.
 *
 * Split out of `ChatWidget` for one reason only - none of it is needed until the operator
 * opens the panel. The launcher stays in the initial shell and this module is fetched on
 * first open, so every protected page stops downloading and parsing the conversation
 * machinery it never runs. Nothing else changed: the markup, the classes, the aria wiring,
 * the privacy warning, the API contract and the focus model are the ones the widget already
 * had.
 *
 * Conversation state lives HERE rather than in the launcher, and survives close/reopen
 * because the launcher keeps this component mounted once it has been opened. That matches
 * the previous behaviour exactly - the messages array outlived a close before this split
 * too, and it could not change while the panel was shut.
 *
 * The panel marks a log region with polite announcements: streaming assistant text is read
 * as it arrives rather than being spelled out all at once. The composer is focused when the
 * panel opens; the launcher owns returning focus to itself on close. Escape closes it. The
 * `print:hidden` matches the rest of the chrome - the widget is a screen convenience and
 * must never leak between the page and a printout.
 */
...
export function ChatPanel({
  ...
  /** Whether the panel is expanded. False renders nothing, exactly as the closed widget did. */
  isOpen: boolean;
  /** Owned by the launcher, which points `aria-controls` at it. */
  panelId: string;
  onClose: () => void;
  /**
   * Announced to the launcher once this module has mounted (CLINIC-PERF-01-R1).
   *
   * It is what retires the launcher's first-open placeholder and latches the panel as
   * rendered, so a later close keeps this component mounted and the conversation and any
   * half-typed draft survive it.
   */
  onReady?: () => void;
}) {
  ...
  // Mount signal for the launcher. Fires once, after the lazily loaded module is live.
  useEffect(() => {
    onReady?.();
  }, [onReady]);

  // Focus enters the composer when the panel opens. Returning it to the launcher on close is
  // the launcher's own effect, because the launcher owns that button's ref.
  useEffect(() => {
