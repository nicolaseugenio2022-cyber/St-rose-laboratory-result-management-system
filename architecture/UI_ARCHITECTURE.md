...
- **Abnormal Result Warning Presentation**: Visual warning badges display inside the encoding form interface only; **warnings MUST NEVER alter official printed report layouts**, except for the universal `HIGH` / `LOW` output marker defined by INV-009: every rendered result evaluating to `High` prints a bold red `HIGH` (`#B91C1C`), every `Low` a bold royal-blue `LOW` (`#1D4ED8`), on every examination capable of producing those outcomes, CBC included and with no template exceptions. The marker is projected generically from the result's existing `evaluationOutcome`; the complete word, not the colour, carries the meaning, and it is right-aligned on one line inside the existing RESULT column without adding a column, joining the result value or changing row height. No other encoding warning reaches official output, and the Encoding badges keep their existing behaviour and colours.
