...
- **Evaluation Behavior**:
  ...
  - Visual abnormal warnings appear in the encoding interface **only**. **Warnings must NEVER alter official printed report layouts**, with the single narrow exception recorded in INV-009: the compact `H` / `L` interpretation marker projected from a result's existing `evaluationOutcome`.
