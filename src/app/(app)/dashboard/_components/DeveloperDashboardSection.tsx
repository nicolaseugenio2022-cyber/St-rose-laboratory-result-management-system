...
function DetailRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    // Stacked below `sm`, paired from `sm` up. A diagnostic value - a timestamp, a
    // Supabase status string, a runtime version - is the part that matters, and on a
    // phone the shared row left it a narrow right-aligned column that wrapped into a
    // ragged two or three lines beside its label. Stacking gives it the full row width
    // and puts it under its own label; the compact desktop density is unchanged.
    <div className="flex flex-col gap-0.5 px-3.5 py-1.5 sm:flex-row sm:items-baseline sm:justify-between sm:gap-3">
      <span className="shrink-0 text-[11px] text-brand-text-muted">{label}</span>
      {/* Wraps rather than truncating: an ellipsis destroys the part that matters. */}
      <span className="min-w-0 break-words text-xs font-medium text-brand-text sm:text-right">{value}</span>
    </div>
  );
}
              ...
              {recentActivity.map((log) => {
                ...
                return (
                  <div key={log.id} className="flex items-start justify-between gap-3 px-3.5 py-2">
                    ...
                    {/* The shared Badge, rather than the same chip retyped here: these two
                        treatments were a hand-rolled copy of its `warning` and `neutral`
                        variants and had already drifted on ring colour. */}
                    <Badge variant={isDenial ? "warning" : "neutral"} size="md" className="shrink-0">
                      {log.category}
                    </Badge>
                  </div>
                );
