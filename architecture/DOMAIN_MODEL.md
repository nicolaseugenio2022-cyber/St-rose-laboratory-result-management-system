...
- **Evaluation Behavior**:
  ...
  - Visual abnormal warnings appear in the encoding interface **only**. **Warnings must NEVER alter official printed report layouts**, with the single narrow exception recorded in INV-009: the `HIGH` / `LOW` interpretation marker projected from a result's existing `evaluationOutcome`.
...
| **INV-009** | Reference rule abnormal warnings affect visual encoding indicators only and MUST NEVER alter official printed report layouts, with one approved exception: official output MAY display an abnormal-result interpretation marker derived **exclusively** from the result's existing `evaluationOutcome` — `High` renders the complete word `HIGH`, `Low` renders the complete word `LOW`, and `Normal`, `Abnormal`, `Invalid`, `Entered` and `NoEvaluation` render nothing. `High` renders a bold red `HIGH` in `abnormalHigh` (`#B91C1C`) and `Low` a bold royal-blue `LOW` in `abnormalLow` (`#1D4ED8`); the complete word carries the meaning, so nothing depends on colour. The marker is right-aligned inside the existing RESULT column on one line, adds no fourth column, is never concatenated into the clinical result value, and changes no row height. **Every** rendered numeric result evaluating to `High` or `Low` receives its marker, on every registered examination capable of producing one, Complete Blood Count included. There are no active template suppression exceptions. Qualitative examinations that cannot reach `High` or `Low` are unaffected. The marker is presentation only: it never recomputes or reinterprets a result, and completed reports project the outcome already frozen in their snapshot. Every other encoding warning remains encoding-only. | Enforced by Report Rendering isolation principles and the shared standard `composeResultGrid` path |
