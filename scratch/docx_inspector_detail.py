    ...
    with zipfile.ZipFile(path, 'r') as z:
        ...
        if sectPr is not None:
            ...
            if pgMar is not None:
                ...
                print(f"MARGINS (pt): Top={top}, Bottom={bottom}, Left={left}, Right={right}")
                print(f"MARGINS (mm): Top={top*0.352778:.2f}mm, Bottom={bottom*0.352778:.2f}mm, Left={left*0.352778:.2f}mm, Right={right*0.352778:.2f}mm")
        ...
        for h in [f for f in z.namelist() if f.startswith('word/header') or f.startswith('word/footer')]:
            ...
            print(f" {h} -> Text: {' '.join(texts)} | Images count: {len(images)}")
        ...
        for idx, child in enumerate(body):
            ...
            if tag == 'p':
                ...
                print(f"P[{idx}] align={align:6s} spBefore={spacing_before}pt spAfter={spacing_after}pt fonts={fonts} sz={sizes} bold={bolds}{img_str}: '{p_text}'")
                ...
                print(f"TBL[{idx}] {len(rows)} rows | Borders: {borders} | GridCol Widths (pt): {col_w}")
                for r_idx, row in enumerate(rows):
                    ...
                    print(f"   Row {r_idx:2d}: " + " | ".join(cell_info))
