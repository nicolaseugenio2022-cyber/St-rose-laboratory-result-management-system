...
Completed History has since been published. **The active objective is Milestone 6 production hardening**, which as of 2026-08-23 has delivered the read transport policy, the request-scoped authentication resolution, the post-hydration read-waterfall removals, and a UI consistency program across the workspace, the directories and the History and Audit viewers. Those are recorded in the dated sections at the end of this document, which supersede the paragraph below where they disagree.
...
## Personnel Directory — P3 Signature Management (2026-08-19)

**Pathologist signature management is complete and live-accepted.** Admin may upload, replace and remove an optional Pathologist signature; Medical Technologists can never carry one. Signature assets live in a private Supabase Storage bucket and are delivered only through an authenticated proxy.

**No custom access token and no HMAC.** The previously unused `generateSignatureAccessToken` minted an unsigned, forgeable `base64url(path:expiry)` value and was **deleted** rather than hardened; it had zero callers and was never reachable. Both signature consumers — the preview image element and the PDF exporter's `fetch` — are same-origin browser requests that carry the session cookie automatically, so an authenticated proxy is sufficient, and `SECURITY_MODEL.md` §9 explicitly permits that delivery mode. The proxy authenticates the session, admits only `Admin` and `User`, validates a strict canonical lowercase UUID object path, and then proves the requested path is referenced by either the current `personnel.signature_image_url` or a frozen `report_signatories.signature_image_url` before streaming bytes with the server-only credential. That reference check is what prevents bucket enumeration by an otherwise-legitimate authenticated caller.

Object paths are server-generated and immutable — `personnel/<personnelUuid>/<objectUuid>.png`. Replacement always writes a **new** object and never overwrites, which is the mechanism that keeps historical reports resolving their original image; removal clears only the current personnel pointer and retains the object. The client never supplies a path, filename or URL, and `signatureImageUrl` remains absent from every client-facing schema. PNG is enforced by magic bytes rather than by the client-supplied MIME type, with a 2 MB application cap. **No migration, DDL, SQL or bucket-configuration change was made by the implementation agent.**

**All final deterministic gates PASS** on the published candidate: `tsc --noEmit`, `next lint`, `next build`, `verify-personnel-signatures.ts` (47 assertions), `verify-personnel-directory.ts`, checkpoint C1 and checkpoint M6C.

**The live bucket is fully compliant**, verified directly against `ruxsmcypeisbkotibzfs`: `personnel-signatures` exists, `Public = false`, allowed MIME `image/png`, maximum size 2 MB, and no anon or authenticated direct-read Storage policy. The last of these was proven against a real uploaded object with a positive control — the service credential downloaded it while the anon credential was denied and the public URL returned 400.

**Live acceptance ran against a production build**, exercising the real Server Actions through the real guards. Valid PNG upload passed. A near-2 MB upload passed: a 1,900,085-byte PNG base64-encodes to 2,533,448 bytes and transported successfully beneath the 3 MB Server Action ceiling, reaching the application-level 2 MB validation rather than being pre-empted by the framework. Non-PNG content and oversize files were both rejected by application validation with no object, no pointer and no audit written. Admin and User both fetched a referenced signature through the proxy; Developer, anonymous callers, unreferenced valid-shaped paths and malformed paths were all denied. MedTech signature upload and every Developer mutation were refused server-side. An ordinary Pathologist detail edit preserved the non-null signature, closing the P2 residual that had never been live-testable. Replacement created a new immutable object while retaining the old one, and removal cleared only the personnel pointer. The expected `PersonnelCredential` audits — `PersonnelSignatureUploaded`, `PersonnelSignatureReplaced` carrying `previousObjectPath`, and `PersonnelSignatureRemoved` — and `SecurityDenial` / `SignatureAssetAccessDenied` events were observed, all with `target_role: null`.

**One acceptance criterion is recorded as an accepted residual rather than fully live-proven.** The *positive* historical leg — an object referenced only by a frozen `report_signatories` row remaining fetchable — was not exercised, because no completed report currently carries a non-null frozen signature and direct SQL is prohibited. What is proven: `report_signatories` stayed **byte-identical** by SHA-256 across upload, replace and remove; frozen-reference propagation is structurally verified, since the stored string is copied verbatim from `personnel.signature_image_url` and the proxy reconstructs it with the identical expression; and the proxy's `report_signatories` query is **live-proven to execute**, returning a 404 rather than the 500 the code raises on query failure. The untested element is therefore a single boolean on a query already shown to run, comparing a string already shown to match. The failure mode degrades gracefully under the existing `OmitImage` policy. **Verify the first real completed report carrying a Pathologist signature after publication.**

**Remaining residuals**, none blocking: unsaved personnel text-field edits are reset when a signature mutation refreshes `initialData`; two orphaned acceptance objects are retained in the bucket; duplicate dead signature schemas remain in `personnelValidation.ts`; a TOCTOU window remains between the final Pathologist role recheck and the pointer write, with the audit recording the pre-recheck role; the proxy performs a full-table personnel lookup per image request; anonymous GETs to the proxy amplify denial-audit rows; a removed signature stays in the private browser cache for up to five minutes; a replacement of a legacy non-proxy URL is audited as `Uploaded` rather than `Replaced`; and audit writes remain non-atomic with the mutations they record, which is the existing project-wide residual.

**Documentation corrections recorded in this synchronization.** The P3 handoff's bucket preflight was initially wrong: all five checks were first reported as confirmed, but live acceptance failed with `Bucket not found` and a read-only probe showed zero buckets in the project. The bucket was then created manually, a second verification found the MIME restriction still absent, and it too was corrected before all five properties verified. The handoff also wrongly described `src/lib/supabase/client.ts` as unused — it retains a consumer in `src/repositories/supabase-auto-suggestion-repository.ts`, which an earlier `@/`-prefixed grep missed. `PRODUCTION_DEPLOYMENT_ARCHITECTURE.md` §5.2 now describes the authenticated private proxy with no custom token, matching its own diagram, which had always shown session verification.

## Login Lockout Retry Countdown (2026-08-19)

**The login form now shows how long remains during a lockout instead of an indefinite "try again later".** The countdown is complete and live-accepted.

**The lockout policy is unchanged** — 6 failures, a 15-minute window, a 15-minute lockout, and the existing progressive cooldowns. The countdown reuses `LoginRateLimiter.assertAllowed` rather than restating any threshold, so the displayed value cannot drift out of step with the policy the login path enforces.

**The server-side limiter remains authoritative.** `retryAfterMs` is derived entirely server-side; the countdown is display only, and disabling the submit button while it runs is UX alone. Refreshing the page, editing client state, or running the client timer to zero early changes nothing, because `assertAllowed` still executes on the real login path and refuses the attempt. Client manipulation cannot bypass the lockout.

**No audit behaviour changed.** The new read-only action constructs the limiter with its optional lockout dependency deliberately omitted; `assertAllowed` guards its entire side-effecting tail behind `if (!this.lockouts) return`, so the status path performs no `releaseExpiredLockout` mutation and emits no audit event. Successful login, logout, failed login, lockout activation and lockout release audits are all unchanged, and no credential material is added to any audit.

**Both pinned auth files are untouched.** `src/features/auth/authActions.ts` and `src/lib/login-rate-limit.ts` remain byte-pinned by M6C; both SHA-256 pins were recomputed against the published candidate and still match, so no rebaseline and no verifier-assertion edit was required. Because `getClientIp` is private to the pinned action file, the two-line header read is reimplemented in the new module rather than unpinning an auth file to export it.
...
## History and Audit Presentation Hardening (2026-08-21 to 2026-08-22)

**History row presentation was corrected in four commits.** `e995d4d` stopped the accession number, examination date and age/sex metadata breaking mid-value; `007364b` capped the tests column at three template chips with a `+N` indicator so a session carrying many examinations cannot widen the column without bound; `d02d01f` consolidated the identity metadata into one line and hid the examination date at narrow widths, where it is the lowest-priority value and remains available in Preview; `e234ac4` gave the search field a real label and aligned the toolbar controls to one baseline.

**Two verifiers were strengthened from mention to proof.** `0f438a7` made guard assertions establish invocation and ordering rather than the presence of a token, so a guard reduced to a comment no longer satisfies them. `e619dac` replaced the source-text proxy for History reopen gating with an assertion that **renders the shipped component** and inspects what it actually produces — the shared `HistorySessionActions` is the single authorization path for both responsive renderings, so the invariant is now proved behaviourally. This is the mechanism that later caught a real crash during the History/Audit visual slice, described below.

**The Audit viewer gained five presentation capabilities.** `ad927b2` replaced the raw Inspect-JSON control with a labelled event details panel; `0c903f7` humanized event labels and added an Outcome column derived from the source event rather than stored separately; `160d6ae` added active-filter chips and made filter matching semantics explicit in the interface, distinguishing exact raw-identifier matching from substring matching; `03710f8` pinned the resulting presentation invariants in `verify-audit-presentation.ts` and fixed the date-picker icon; `efcdcaf` debounced free-text filter requests to one per typing pause.

**One commit carries a malformed subject.** `a16965d` was committed with a pasted working-tree status line as its message rather than a conventional subject. The change it carries is part of the Audit filter-chip work and is otherwise sound. History has not been rewritten to correct it, and doing so is not proposed: the commit is published and rewriting it would invalidate every downstream hash for a cosmetic gain.

## Authority and Orchestration Rules (2026-08-21)

Three commits changed how agents operate in this repository rather than what the application does. `d2b695b` pointed the orchestration rules at the shared Obsidian vault as a selectively consulted durable store, explicitly not session context. `569ebfa` made Claude the sole agent on this account and required an approved independent reviewer, removing the assumption that work is delegated to other tools. `a50d9de` scoped that independent review to slices that genuinely cross a security boundary rather than treating publication itself as the trigger.

## Milestone 6 — Performance and Transport Program (2026-08-22)

**`d595111` closed a security-reporting defect first.** `loginAction` previously reported infrastructure failures as invalid credentials, which told an operator their password was wrong when the real fault was transport. The generic branch now reports a retryable condition and logs sanitized metadata only, and `InvalidCredentialsError` remains the single class covering wrong password, unknown username and inactive account, so nothing in the change weakens anti-enumeration.

**`b1a94a2` established a single retry owner for reads.** `resilientFetch` wraps the server Supabase client with a deliberately asymmetric policy: GET and HEAD share an 8 s total budget across at most two attempts, each capped at 5 s, retried only for proven transient transport codes or the wrapper's own attempt timeout; POST, PATCH and DELETE are delegated to `fetch` byte-for-byte with no timeout and no retry, because a client-side abort on a write leaves an ambiguous outcome. postgrest-js's own retry layer is disabled through `db.retry: false` so the wrapper is the only retry layer end to end. `38097dd` declared the Node 20.3 runtime floor that policy requires. `d234045` routed server-side suggestion learning through the bounded server client.

**`077eab7` (QA-01R-R3) superseded that attempt limit (2026-09-10).** The paragraph above records what `b1a94a2` established and stands as history. QA-01R-R2 later let an idempotent read retry once when PostgREST answered HTTP 401 with `PGRST303`, but only within the same shared two-attempt allowance. A production login failure on 2026-09-09 justified changing that limit. Correlated Vercel and Supabase gateway logs support the following sequence as the best explanation; it is not directly instrumented proof. The first attempt of the client-IP `auth_attempts` read left no gateway entry and appears to have stalled until the 5 s attempt timeout, which used the only retry. The retry reached the gateway about 5.2 s after the read was issued, was answered 401 with `PGRST303`, and surfaced as a generic infrastructure failure. The current operational limit is three network attempts per read: one initial attempt, at most one retry for a transport failure or attempt timeout, and at most one retry for a confirmed `PGRST303` response, all inside the unchanged 8 s total budget and 5 s per-attempt cap. Writes are still never retried, any other 401, any 403 and any other completed error are returned untouched, and `db.retry` remains `false`.

**Four commits removed post-hydration read waterfalls.** `fc53593` served History and Workspace initial data with the page; `6336835` read recent sessions and their ownership in one query instead of two; `8710df5` server-rendered the initial `/users` directory, replacing a three-read, two-request mount with one request; `793d850` moved workspace auto-suggestion reads to a server action. That last change also fixed availability rather than only latency: `auto_suggestions` RLS denied the previous browser-side read, so the datalist was always empty and now populates. It additionally removed the browser Supabase client from the workspace graph, taking `/workspace` First Load JS from 213 kB to 149 kB with GoTrue and Realtime absent from the initial chunks.

**`85b1fb2` made authenticated request resolution single-pass**, validating the authenticated user once per request and reusing it, keyed by cookie and scoped to the request. **`b874db5` made the authenticated shell fail closed with a retryable account state** when the profile read fails in transit, rather than redirecting to `/login` — a transport failure is not a signed-out session, and the two must not be conflated. `isTransientSupabaseReadFailure` classifies only the narrow envelope shapes the read policy itself can produce, and is deliberately fail-closed: an upstream library change narrows the classification rather than widening it.

## UI Consistency Program (2026-08-22 to 2026-08-23)

**`0a863db`** pinned the sidebar to the viewport and enlarged the authentication mark into one shared `AuthBrandMark`. Making the desktop rail sticky activated a previously inert `z-50`, which would have lifted it above the header's stacking context and over the change-password dialog mounted inside that header with no portal; `lg:z-auto` neutralises it while the mobile drawer keeps its overlay.

**`0b11ed9`** tightened the shared encoding hierarchy across **all 17 registered examinations**, not CBC alone. An examination coverage matrix was generated from the live registry first, which corrected the assumption that CBC is representative: URINALYSIS is the densest case at 12 parameters with four control types and repeatable findings, and CBC exercises only NumericText. The work reclaims roughly 150 px above the first result control at 1920×1080 with no template-code branch anywhere in the shared components.

**`e7dd237`** fixed a root cause with wide consequences: `src/rendering/**` was absent from Tailwind's `content` globs, so twelve classes used only in the live-preview code were never compiled — including `bg-slate-900`, leaving the preview toolbar with no dark background, `bg-emerald-600`, leaving Export PDF unfilled, and `max-h-[calc(100dvh-16rem)]`, leaving the viewport with no height cap. Adding the glob was proved purely additive by building the same candidate with and without it: 752 to 794 selectors, 42 added, none removed. The same commit gave the Print control a disabled affordance it never had, replaced a dead `shadow-xs` that is not a Tailwind v3 utility, and added zoom and page-navigation semantics.

**`6c85d7a`** aligned the User and Personnel directory tables: `Badge` no longer wraps "Medical Technologist" onto two lines, the role filters share one width, `Trash2` replaces the `Power` glyph that Delete previously shared with Deactivate, and every row action carries a subject-specific accessible name.

**`d6bef9d`** brought `/history` and `/audit` onto the same visual system — heading treatment, filter-card surfaces, table-header typography, row rhythm and button language — while deliberately preserving what is route-specific: `px-2.5` horizontal cell padding, `table-fixed` percentage columns, and the card renderings below `md`. During this slice `verify-checkpoint-b4` caught a genuine crash: the new row-subject expression dereferenced `session.demographics` on a fixture that has none. It was corrected to optional chaining before publication, and it is the clearest evidence yet that `e619dac`'s render-based assertions earn their cost.

## P5 — Production First-Login Failure (Parked Inconclusive, 2026-08-23)

**A reported intermittent failure was investigated read-only and could not be reproduced.** Under a production build, the first login immediately after `npm start` was reported to fail with "Unable to sign in right now" while the same credentials succeeded seconds later, logging `errorName: 'object'`.

**Source analysis narrowed six candidate stages to three and eliminated two on proven grounds.** The audit emit is fail-soft and cannot propagate; `createSession` performs no database I/O at all, only token signing and a cookie write, so it cannot produce the observed plain object. The three live candidates are the two `auth_attempts` rate-limit reads, the `user_profiles` credential read, and the mandatory `auth_attempts` insert. Every repository ends with `if (error) throw error`, and postgrest-js converts both a rejected fetch and a completed HTTP error into a plain object, so the existing log cannot discriminate among them.
...
- **`text-brand-text-muted/90` is never emitted.** This Tailwind configuration defines brand colours as bare `var(...)` values, so alpha modifiers on them compile to nothing. `/users` and `/personnel` both use the `/90` form on their subtitles and therefore apply no muted colour at all. The same class of defect exists for `focus:ring-brand-primary/20` and `ring-brand-primary/30`. Nothing is visually broken enough to force a slice, but no new brand-token opacity utility should be introduced.
