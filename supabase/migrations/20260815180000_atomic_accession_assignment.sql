CREATE OR REPLACE FUNCTION resolve_session_accession_number(target_session_id uuid)
RETURNS text
LANGUAGE plpgsql
SECURITY INVOKER
...
BEGIN
  ...
  IF v_accession IS NULL THEN
    v_accession := allocate_accession_number();
  END IF;
...
REVOKE EXECUTE ON FUNCTION resolve_session_accession_number(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION resolve_session_accession_number(uuid) TO service_role;

CREATE OR REPLACE FUNCTION persist_session_report_tree(
  target_session_id uuid,
  reports jsonb,
  p_skip_empty_results boolean,
  p_write_signatories boolean
)
RETURNS void
LANGUAGE plpgsql
SECURITY INVOKER
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM jsonb_array_elements(COALESCE(reports, '[]'::jsonb))
...
REVOKE EXECUTE ON FUNCTION persist_session_report_tree(uuid, jsonb, boolean, boolean) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION persist_session_report_tree(uuid, jsonb, boolean, boolean) TO service_role;
...
DROP FUNCTION IF EXISTS complete_patient_report_session(jsonb);
